...
const participants = Object.keys(participantsJson).map(id => ({
    ...
    manv: participantsJson[id].manv, // Mã nhân viên
    chucvu: participantsJson[id].chucvu, // Chức vụ
}));

// Set the predefined result here. Use null for random result
const predefinedId = parseInt(assigneeId) || null;
// let predefinedResult = participants.find(p => p.id === predefinedId) || getRandomParticipant();
...
const numRaffleBoxes = value; // Số lượng người trúng giải
...
for (let i = 0; i < numRaffleBoxes; i++) {
    ...
    box.innerHTML = '<div class="raffle-result-text">?</div>';
...
function shuffleNames(box) {
    ...
    // Hiển thị manv khi đang quay
    box.innerHTML = `
        <div class="raffle-result-text">${randomParticipant.manv || '?'}</div>
    `;
}
...
function stopShuffling(box, targetParticipant, interval, index) {
    ...
    // Hiển thị manv | name khi dừng
    const displayText = `${targetParticipant.manv || ''} | ${targetParticipant.name || ''}`;
    const chucvuText = targetParticipant.chucvu || '';
    box.innerHTML = `
        <div class="raffle-result-text">${displayText}</div>
        ${chucvuText ? `<div class="raffle-result-chucvu">${chucvuText}</div>` : ''}
    `;
    finalResults[index] = {
...
function saveRaffleResult() {
    if (finalResults.length === 0) {
        // alert('Raffle is not done yet or no valid results.');
        return;
    }
