...
const raffleTime = parseInt(time) || 5000; // Default 5 seconds
...
const participants = Object.keys(participantsJson).map(id => ({
    ...
    workplace: participantsJson[id].workplace,
...
// Company types
const COMPANY_TYPES = ['newtecons', 'ricons', 'sol'];
    ...
    raffleBoxes.forEach((box, index) => {
        ...
        if (winners[index]) {
            ...
            box.innerHTML = `
                ...
                <div class="winner-workplace">${winner.workplace || ''}</div>
                <div class="winner-company">${winner.company || ''}</div>
            `;
        }
    });
