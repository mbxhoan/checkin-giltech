...
class ClientImport implements ToModel, WithHeadingRow, WithEvents, WithChunkReading, WithStartRow, SkipsOnFailure, SkipsEmptyRows
{
        ...
        if (!empty($this->modelImp)) {
            return [
                BeforeImport::class => function (BeforeImport $event) {
                    ...
                    // Get current number of clients for the event
                    $existingClients = $this->event->clients()->count(); // Assuming $event->clients() relationship exists

                    // Get allowed limit
                    $limit = $this->event->company->limited_clients;

                    if (is_numeric($limit) && ($existingClients + $importRows) > $limit) {
                        $total = $existingClients + $importRows;
                        // Optionally log this error
                        $this->logErrors(1, "Dữ liệu khách hàng ($total) vượt quá giới hạn được phép.");

                        // Stop the import by throwing an exception
                        throw ValidationException::withMessages([
                            'file' => ["Dữ liệu khách hàng ($total) vượt quá giới hạn được phép."],
                        ]);
                    }
                }
            ];
    ...
    public function model(array $row)
    {
        ...
        $customFields = [];
        if(!empty($row['email'])) {
            $row['email'] = strtolower(preg_replace('/\s+/', '', trim($row['email'])));
        }
        ...
        if (isset($this->processedQrcodes[$qrcode])) {
            $this->logErrors($this->currentRow, "Mã qrcode {$qrcode} bị trùng trong file");
            return null;
        }
        ...
        $attributes = [
            ...
            'type'                  => !empty($row['type']) ? trim($row['type']) : (!empty($client) ? $client->type : "NONE"),
        ...
        // $this->modelImp->refresh();
        // $this->modelImp->update([
        //     'total_record' => $this->modelImp->total_record + 1,
        // ]);
        // $this->modelImp->save();

        // Save current progress to cache using import ID
        Cache::put("import_progress_{$this->modelImp->id}", [
    ...
    {
        $msgError = "Lỗi nạp file";

        if (auth()->user()->isSysAdmin()) {
            $msgError = $e->getMessage();
        }

        $this->logErrors($this->currentRow, $msgError);
    }
}
